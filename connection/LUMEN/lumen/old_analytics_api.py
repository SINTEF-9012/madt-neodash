...
def analytics_generate_and_run_code():
    ...
    graph_explorer = ConversableAgent(
        "GraphExplorer",
        system_message = """Your name is GraphOperator. You answer requests by querying a Neo4j database. Generate Cypher queries and use the registered tool to execute the query. 
                          You MUST follow the schema: {DB_SCHEMA}.   
                          Important: For fetching data, you first retrieve the bucket property of DATASOURCE and STATICDATA nodes. """,
        llm_config = current_llm_config,
        code_execution_config=False,
        human_input_mode= "ALWAYS" if DEBUG_MODE else "NEVER"
    )
    ...
    register_function(
        ...
        description = "Query or modify the Neo4J database. You input a CYPHER query, and the output is a list of records returned by the query."
    )
